...
export class MapComponent implements OnInit {
  ...
  /**
   * - 1 First we need to construct/build the mapping object that we can bind to the div container with the
   * id of "map" with the target: "" key
   * - 2 Next is to determine what layers our initial map will contain, layers are essentially the building blocks
   * of any openlayers application, different layers can provide different views or pieces of information.
   * In the case of a TileLayer a tilelayer is a prerendered layer in this case it's just a view of the world
   * with street names, country names ect.
   */
  setupMap() {
    this.map = new Map({
      target: 'map',
      layers: [
        new TileLayer({
          // The attribution containing a link to the OpenStreetMap(OSM) Copyright and License page.
          source: new OSM()
        }),
        this.vectorLayer
      ],
  ...
  addClick(){
    ...
    this.map.on("click", function (e) {
      self.map.forEachFeatureAtPixel(e.pixel, function (feature, layer) {
        self.NameService.setLocoName(feature.values_.name);
      })
    });
  }
  plotDiveSites() {
